#!/usr/bin/env python3
"""
Improved Optimized Model Training Script
...
12. REMOVED recorded_outages lag features (only weather lags are used)
...
- Skips test phase if the test set is empty (e.g., specific EVENT_TYPE missing in test year)
- Proper EVENT_TYPE importance tracking in Unified mode

Usage:
  python train_optimized_final.py data.parquet Unified LightGBM 1 outputs/ --resume
  python train_optimized_final.py data.parquet Ensemble XGBoost 1 outputs/ --resume
"""
...
class Config:
    ...
    def __post_init__(self):
        base_columns = [
            ...
            'EVENT_TYPE', 'Total DVMT', 'Population', 'LAI', 'event_no'
        ]
        weather_features = [
        ...
        # Only add weather feature lags - NO recorded_outages lag
...
class DataLoader:
    ...
    def load_and_prepare(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, List[str]]:
        ...
        # Split train/test
        train_df = df[df['Date'].dt.year < self.config.TRAIN_YEAR_CUTOFF].copy()
        test_df = df[df['Date'].dt.year == self.config.TRAIN_YEAR_CUTOFF].copy()
    ...
    def seed_ensemble_predict(self, train_df: pd.DataFrame, test_df: pd.DataFrame, features: List[str],
        ...
        for test_num in range(1, self.config.N_TEST_PREDICTIONS + 1):
            ...
            try:
                ...
                pred_df = test_df[['Date', 'CNTY_NM', 'EVENT_TYPE', self.config.TARGET]].copy()
        ...
        combined_predictions = all_predictions[0][['Date', 'CNTY_NM', 'EVENT_TYPE', self.config.TARGET]].copy()
...
class ResultsSaver:
    ...
    def save_summary_statistics(self, predictions: pd.DataFrame):
        ...
        summary = predictions.groupby(['Date', 'CNTY_NM', 'EVENT_TYPE']).agg({
        ...
        summary.columns = ['Date', 'CNTY_NM', 'EVENT_TYPE', 'Actual',
...
def run_single_model(df: pd.DataFrame, config: Config, base_output_dir: Path, resume: bool = False):
    ...
    # If there's no test data (e.g., specific EVENT_TYPE missing in the test year), skip testing later.
    no_test_rows = test_df.shape[0] == 0
    if no_test_rows:
        logger.warning("No test rows found for this configuration (likely specific EVENT_TYPE missing in test period). Will skip testing phase.")
...
def main(
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("IMPROVED OPTIMIZED MODEL TRAINING - FINAL FIX")
...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print("Usage: python train_optimized_final.py <parquet_file> <model_type> <algorithm> <lag> [output_dir] [--resume]")
